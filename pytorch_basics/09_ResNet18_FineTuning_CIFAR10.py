...
# 기존 pretrained parameters freeze
for parameter in model.parameters():
    parameter.requires_grad = False

# ImageNet 1000 classes → CIFAR10 10 classes
in_features = model.fc.in_features
model.fc = nn.Linear(in_features, 10)

# 마지막 ResNet block 학습 가능하게
for parameter in model.layer4.parameters():
    parameter.requires_grad = True



## 입력도 최대한 그 모델이 학습할 때 봤던 형태와 비슷하게 만들어줌
## 이 weight를 사용할 때 권장되는 preprocessing을 가져옴
## 사실상 이렇게 해줌: 
# transforms.Compose([
...
train_dataset = datasets.CIFAR10(root=DATA_DIR, train=True,  # 학습 데이터
                               transform=preprocess, # pretrained ResNet18에 맞는 resize/crop/normalize 적용
                               download=True)
...
def model_train(dataloader, model, loss_function, optimizer):
    ...
    for images, labels in dataloader:
        ...
        x_train = images.to(DEVICE) # [64, 3, 224, 224]
