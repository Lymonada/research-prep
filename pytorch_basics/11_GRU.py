...
class MyGRUModel(nn.Module):
    ...
    def forward(self, data):
        ...
        outputs, h_n = self.gru(data, h0) # gru의 리턴값은 마지막 레이어의 모든 timestep의 hidden state를 모아놓은 outputs -> shape은 [B, T, H × D]
...
def model_train(dataloader, model, loss_function, optimizer):
    ...
    for sequences, targets in dataloader:
        ...
        ## 5일짜리 sequence를 GRU에 입력하여 다음 날 Close 예측
...
plt.title("Samsung Electronics Stock Price Prediction (GRU)")

plt.grid()
plt.legend()

plt.savefig(
    PLOT_DIR / "GRU_prediction.png",
    dpi=150,
    bbox_inches="tight"
)
